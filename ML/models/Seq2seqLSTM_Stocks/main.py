...
def correct_sql_by_gpt3(user_input, input_query):
    ...
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "너는 이제부터 Seq2seq LSTM에서 나온 SQL Query를 교정해주는 역할이야. 답변은 다 생략하고 무조건 수정된 SQL Query만 출력해줘. 그리고 Query를 보내는 데이터 베이스의 table은 2개로 주가 관련된 질문을 물어보면 from StockDailyData로 생성해주고 부채나 자산을 물어보면 from AnnualFinanceStatement로 생성해줘."},
            {"role": "system", "content": "Query를 보내는 데이터베이스의 StockDailyData 테이블의 column은 stck_bsop_date, stck_clpr, stck_oprc, stck_hgpr, stck_lwpr, company로 총 7개야. (stck_bsop_date = 기간, stck_clpr = 주식 종가, stck_oprc = 주식 시가, stck_hgpr = 주식 최고가, stck_lwpr = 주식 최저가를 뜻 해)"},
            {"role": "system", "content": "Query를 보내는 데이터베이스의 AnnualFinanceStatement 테이블의 column은 label_ko, class1, class2, Date, finance_statement, company로 총 6개야."},
            {"role": "system", "content": "AnnualFinanceStatement에 요청하는 Query를 만들때, class2가 부채총계 or 자산총계야"},
            {"role": "user", "content": "삼성전자의 자산 보유량을 알려줘."},
            {"role": "assistant", "content": "Select finance_statement, class1, Date, company FROM AnnualFinanceStatement WHERE (company = '삼성전자') and (class2 = '자산총계')"},
