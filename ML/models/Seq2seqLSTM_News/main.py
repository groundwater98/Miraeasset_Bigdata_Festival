...
def correct_sql_by_gpt3(user_input, input_query):
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            ...
            {"role": "system", "content": "Query로 접근하려는 데이터베이스의 테이블 이름은 NaverNews니까 Query를 만들 때, FROM NaverNews는 고정해줘."},
            {"role": "user", "content": "삼성전자의 최신 뉴스를 요약해줘."},
            {"role": "assistant", "content": "SELECT Company, Title, Link, Description, Pubdate FROM NaverNews WHERE (Company = 삼성전자) order by Pubdate DESC LIMIT 1"},
